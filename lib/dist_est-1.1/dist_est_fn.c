...
void objfun_harwell(int *nrates, double *wt, double *objf, double *objgrd)
{
  int i, j;
  double fval;

  *objf = 0;
  for(j = 0; j < *nrates; j++) objgrd[j] = 0.0;
    
  for(i = 0; i < lrout_endsite; i++){
    fval = 0.0;
    for(j = 0; j < *nrates; j++)
      fval += wt[j]*lrout_likmat[i + j*lrout_endsite];
    *objf -= lrout_count[i]*log(fval);      
    for(j = 0; j < *nrates; j++)
      objgrd[j] -= lrout_count[i]*lrout_likmat[i + j*lrout_endsite]/fval;
  }
}
void optrates_harwell(int endsite, int nrates, int *count, double *likmat, 
		      double *wt, double *rate)
{
  int iprint = 0, info = 0, two = 2;
  int nact, i;
  long *iact;
  double *a, *xl, *xu, *par, *w;
  double b[2] = {1.0, 1.0}, acc = 0.0;

  lrout_endsite = endsite;
  lrout_likmat = likmat;
  lrout_count = count;

  iact = (long *) malloc((size_t) (2*nrates+2)*sizeof(long));
  a = (double *) malloc(2*nrates*sizeof(double));
  xl = (double *) malloc(nrates*sizeof(double));
  xu = (double *) malloc(nrates*sizeof(double));
  par = (double *) malloc((nrates*2 + 2)*sizeof(double));
  w = (double *) malloc((2+11*nrates+nrates*nrates)*sizeof(double));
  ...
  ve11ad_(&nrates, &two, &two, a, &nrates, b, xl, xu, wt, &acc, iact, &nact, par,
	  &iprint, &info, w, objfun_harwell);
...
void mode_rate_estf(int endsite, int nrates, double *rate, double *wt, 
